...
do
    ...
    case $key in
        ...
        ;;
        -*|--help)
        echo "Type script as:$(basename $0) [-v |--verbose] [interfacename]"
        exit 1
        ;;
        *)
        interface="$1"
...
#####
interfaces=$(ifconfig -a | cut -d ' ' -f1 | cut -d ':' -f1 | grep -v '^$' | grep -v 'lo')

for interface in $interfaces; do
	echo "Collecting the interfaces information"
